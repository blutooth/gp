...
class Stationary():
    ...
    def __init__(self, input_dim, variance=None,lengthscales=None ):
        ...
        if variance==None:
            self.variance=tf.square(tf.Variable(1.0,dtype=tf.float64))+0.0001
        else:
            self.variance=variance
        if lengthscales==None:
            self.lengthscales=0.001+tf.square(tf.Variable(tf.ones([input_dim],dtype=tf.float64),dtype=tf.float64))
        else:
...
class Kern:
    ...
    def __init__(self, dim, var=1.0, lenSc=None):
        ...
        if lenSc is None:
            lenSc=tf.Variable(np.ones(dim),dtype=tf.float64)
        self.lenSc=lenSc
